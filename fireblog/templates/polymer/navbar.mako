...
${parent.header_toolbar()}
    ...
    % if request.matched_route.name in ('view_post', 'home'):
        ...
        <div><a href="${request.route_url('change_post', postname = title, id = id, action = 'edit')}"><paper-icon-button icon="create"></paper-icon-button></a></div>
        % endif
        % if request.has_permission('del'):
        <div><a href="${request.route_url('change_post', postname = title, id = id, action = 'del')}"><paper-icon-button icon="delete"></paper-icon-button></a></div>
        % endif
    % endif
